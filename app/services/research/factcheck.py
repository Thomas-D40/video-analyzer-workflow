...
async def search_google_factcheck(
    ...
    async with httpx.AsyncClient(timeout=GOOGLE_FACTCHECK_TIMEOUT) as client:
        response = await client.get(GOOGLE_FACTCHECK_BASE_URL, params=params)
    ...
    if "claims" in data:
        for claim in data.get("claims", [])[:max_results]:
            ...
            claims.append({
                ...
                "published": review.get("reviewDate", "")[:10],
